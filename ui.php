...
<body>
<button id="totalPriceBtn" class="btn btn-success btn-lg" style="display: none; position: fixed; top: 0; right: 0;">price</button>
<p id="testingCart">.</p>
<!--START NEW PAGE-->
    ...
    <div class="row">
        ...
        <div class="col-sm-1" >   
            ...
            <ul class="nav nav-tabs nav-stacked" style="height: 90%;">
                ...
                <li ><a  data-toggle="tab" href="#priceSection">Get Price <span style="visibility: hidden;">spacing</span></a></li>
        ...
        <div class="col-sm-3" style="height: 90%;">
            <div class="tab-content" >
                ...
                <div id="priceSection" class="tab-pane fade"> 
                    ...
                    <button id="productPickerBtn" type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#productPicker" onclick="setDesign();">Pick Products</button>
                    ...
                    <div class="modal-dialog modal-lg">
                      <div class="modal-content">
                        ...
                        <div class="modal-footer">
                          <button type="button" class="btn btn-default" data-dismiss="modal" onclick="addingToCart = true; uploadEx(); ">Add To Cart</button>
                        </div>
                      </div>
                    </div>
                    </div>
                    ...
                    <div >
                        ...
                        <button type="button" id="cartTotal" class="btn btn-success" style="display: none;" data-toggle="modal" data-target="#cartModal" onclick="getCheckoutCart();"></button>
                        <!--CART MODAL-->
                        ...
                        <script type="text/javascript"> 
                            ...
                            function addToCart(){
                                ...
                                cell0.innerHTML = "<div style='width:100px;  background-image:"+product+"; background-repeat: no-repeat; background-size: cover; background-position: center center;'>"+
                                ""+"<img src='"+design+"' style=' width: 50% !important; height: 75% !important; display: block; margin: auto; position: relative; top: 10% !important;'>"+"</div>";
                                cell1.innerHTML = "";
                                cell2.innerHTML = "";
                                ...
                                cell5.innerHTML = "<input type='hidden' name='product_"+rowNum+"' value="+product+">";
                                cell6.innerHTML = "";//"<input type='hidden' name='design_"+rowNum+"' value="+design+">";
                            ...
                            function getCheckoutCart(){
                            	...
                            	alert("current item : " + temp);
                            	var str = "";
                            	for (var i = 0; i < cart.length; i++) {
                            		str += ' | | ' + JSON.stringify(cart[i]);
                            	}
                            	document.getElementById('testingCart').innerHTML = str; */
                            	var tempTotal = 0.0;
                            	...
                            	//alert('checkout cart finished');
                                /*var source = table;
                                var destination = document.getElementById('checkoutCart');
                                var copy = source.cloneNode(true);
                                copy.setAttribute('id','checkoutCart');
                                destination.parentNode.replaceChild(copy, destination);
                                //LOOPING THROUGH TO MAKE SURE ALL DATA IS VISIBLE
                                var checkoutCart = document.getElementById('checkoutCart');
                                for(var i = 0; i < checkoutCart.rows.length; i++){
                                    for(var j = 0; j < 7; j++){
                                       if( checkoutCart.rows[i].cells[1].innerHTML.indexOf('<span style="display:none;">') > -1 || checkoutCart.rows[i].cells[2].innerHTML.indexOf('<span style="display:none;">') > -1 ){
                                            checkoutCart.rows[i].cells[1].innerHTML = checkoutCart.rows[i].cells[1].innerHTML.substring( 28 );
                                            checkoutCart.rows[i].cells[2].innerHTML = checkoutCart.rows[i].cells[2].innerHTML.substring( 28 );
                                        }
                                    }
                                }*/
                            ...
                            function removeFromCart(element){
                                ...
                                /*var temp  = JSON.stringify(cart.splice(rowNum - 1,1));
                                document.getElementById('testingCart').innerHTML = temp;*/
                                //alert('cart.length : ' + cart.length);
                                rowNum--;
                                getCartTotal();
                            }
                            function getItemPrice(){
                                ...
                                item.total = Number(yxs) + Number(ys) + Number(ym) + Number(yl) + Number(yxl) + Number(s) + Number(m) + Number(l) + Number(xl) + Number(xxl) + Number(xxxl) + Number(xxxxl) + Number(xxxxxl);
                                return Number(yxs) + Number(ys) + Number(ym) + Number(yl) + Number(yxl) + Number(s) + Number(m) + Number(l) + Number(xl) + Number(xxl) + Number(xxxl) + Number(xxxxl) + Number(xxxxxl);
                            }
                            function getCartTotal(){
                                ...
                                for (var i = 0; i < cart.length; i++) {
                                	total += cart[i].total;
                                	//alert('i : '+i+' total : ' + total + ' item.total : ' + cart[i].total);
                                }
                                for (var i = 0; i < columns.length; i++) {
                                    var temp = columns[i].innerHTML;
                                    if(temp.indexOf('<h6>$') > -1){
                                        //total = Number(temp.substring(temp.indexOf('$') + 1,temp.indexOf('</'))) != NaN ? total + Number(temp.substring(temp.indexOf('$') + 1,temp.indexOf('</'))) : "Error";
                                    }
                                }
                ...
                <!--SHARE SECTION-->
                    ...
                    <script type="text/javascript">
                        ...
                        function getImage(fileAddress){
                            ...
                            designArray.push(imageLocation.substring(0, imageLocation.length - 6) + picture);	//saving the most recent design for check out purposes 
